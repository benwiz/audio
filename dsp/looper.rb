...
# Start, stop, delete based on current state.
def master()
  # If not currently recording and there are buffers already recorded
  if !get(:curr_buffer_id) && get(:buffer_ids).size > 0
    puts "Delete"
    delete_buffers()
    # Cycle play/pause (TODO: I should not need to do this)
    set :play, false
    sleep 100 # Oof having to sleep is not good
    set :play, true
  else
    # If there is no recording currently happening
    if get(:curr_buffer_id) == nil
      puts "start record"
      start_record_audio()
    else
